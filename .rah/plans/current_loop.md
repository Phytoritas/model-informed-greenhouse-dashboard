...
## Exact Restart Step
1. Continue on issue `#3` and branch `feat/3-replace-concept-demo-rtr-windows-with-grower-approved-periods`; issues `#8` and `#10` are now complete and merged, so the remaining active gap is again the blocked RTR calibration-input upgrade.
