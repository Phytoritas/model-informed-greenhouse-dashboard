...
## Exact Restart Step
1. Continue on issue `#3` and branch `feat/3-replace-concept-demo-rtr-windows-with-grower-approved-periods`; issue `#6` / PR `#7` are complete, so the remaining active gap is again the blocked RTR calibration-input upgrade.
