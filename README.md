...
- Expand the current rule-engine mainline into a fuller nutrient calculator while preserving the landed pesticide safety baseline and the nutrient safety baseline around negative-input rejection, provisional partial-input drafts, bounded drain-feedback target shifts, additive residual-safe bundle alternatives, and bundle-level guardrail rechecks.
