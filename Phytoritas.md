...
- Issues `#6` / `#8` / `#10` / `#12` and PRs `#7` / `#9` / `#11` / `#13` are complete, so the active follow-up loop returns to issue `#3` and branch `feat/3-replace-concept-demo-rtr-windows-with-grower-approved-periods`.
- The synced issue `#3` branch now carries the merged KAMIS live produce price panel, the 14-day actual + 3y/5y/10y seasonal-normal trend overlay, the issue `#10` fallback/labeling hardening, the issue `#12` dashboard layout rebalance, and the latest CI/runtime maintenance.
