...
function Assert-GhAuth {
    ...
    if ($exitCode -ne 0) {
        $details = (($statusOutput | ForEach-Object { [string]$_ }) -join [Environment]::NewLine).Trim()
        if (-not $details) {
            $details = "gh auth status returned a non-zero exit code."
        }
        Write-Warning ("gh auth status could not be verified cleanly. Continuing so the next gh command can surface the actionable failure. Details: {0}" -f $details)
    }
}
